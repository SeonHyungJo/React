...
고차 컴포넌트를 알기 전에 알아두면 좋은 개념이 있다. 고차 함수이다. 이와 관련된 글을 읽어보는 것을 추천한다.
...
고차 컴포넌트는 React의 컴포넌트적 성격에서 나타나는 패턴이다.

구체적으로 고차 컴포넌트는 컴포넌트를 받아서 새로운 컴포넌트를 반환하는 함수이다.
...
기존에는 컴포넌트가 UI를 props로 반환한다면, 고차 컴포넌트는 컴포넌트를 다른 컴포넌트로 변환해서 반환한다.

우리가 Redux에서 많이 보았던 `connect()`가 고차 컴포넌트 중 하나이다.

예전의 리액트에서는 **크로스 커팅**을 해결하기 위해서 Mixin 패턴을 권장을 했었다. 현재는 Mixin 패턴이 적합하지 않다는 것을 깨닫고 더이상 권장하지 않는다.
...
블로그 포스트를 구독하기 위해 비슷한 패턴으로 컴포넌트를 작성한다.
...
위의 로직을 한 곳에 정의하고 많은 컴포넌트에서 공유할 수 있게하는 추상화가 필요하다. 이럴 때 고차 컴포넌트가 탁월하다.
...
고차 컴포넌트는 데이터가 사용되는 이유 및 방법과 연관이 없으며 래핑된 컴포넌트는 데이터가 어디서부터 왔는지와 연관이 없다.
...
컴포넌트와 마찬가지로 withSubscription 과 래핑된 컴포넌트 간 맺음은 완전히 props 기반입니다. 이렇게하면 래핑된 컴포넌트에 동일한 props를 제공한다면 다른 고차 컴포넌트를 쉽게 교차할 수 있습니다. 예를 들어 데이터를 가져오는 라이브러리를 변경하는 경우 유용하게 사용할 수 있다.
